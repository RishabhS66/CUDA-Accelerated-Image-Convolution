...
__global__ void convolution(double *I0, double *F, double *O) {
    ...
    __shared__ double I_shared[C][BLOCK_SIZE][BLOCK_SIZE];
        ...
        for (int c = 0; c < C; ++c) {
            ...
            for (int i = 0; i < FH; ++i) {
                ...
                for (int j = 0; j < FW; ++j) {
                    ...
                    if(yi < by * TILE_SIZE + P || yi >= (by+1) * TILE_SIZE + P
                    || xi < bx * TILE_SIZE + P || xi >= (bx+1) * TILE_SIZE + P){
                        sum += F[k * C * FH * FW + c * FH * FW + (FH - 1 - i) * FW + (FW - 1 - j)] * I0[c * (H + 2 * P) * (W + 2 * P) + yi * (W + 2 * P) + xi];
                    } else {
                        sum += F[k * C * FH * FW + c * FH * FW + (FH - 1 - i) * FW + (FW - 1 - j)] * I_shared[c][ty + i - P][tx + j - P];
                    }
                }
            }
        }
...
int main() {
    ...
    printf("C2_checksum: %.3f, C2_execution_time: %.3f ms\n", checksum, total_time);
